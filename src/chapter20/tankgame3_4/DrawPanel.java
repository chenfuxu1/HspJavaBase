...
import chapter20.tankgame3_4.play.AePlayWave;
import chapter20.tankgame3_4.shot.EnemyShot;
import chapter20.tankgame3_4.shot.HeroShot;
import chapter20.tankgame3_4.shot.ShotParent;
import chapter20.tankgame3_4.tank.EnemyTank;
import chapter20.tankgame3_4.tank.HeroTank;
import chapter20.tankgame3_4.tank.TankParent;
...
public class DrawPanel extends JPanel implements KeyListener, Runnable {
    ...
    // 击毁敌方坦克数量
    private int mDeadTankNum = 0;
    private EnemyTank mRecordEnemyTank; // 记录玩家成绩的敌方坦克模板
    private AePlayWave mAePlayWave;
    private static final String MUSIC_FILE_PATH = "src\\music.wav";

    public DrawPanel(String choose) {
        // 初始化我方坦克
        mHeroTank = new HeroTank(150, 300, Constants.TOP_DIRECTION);
        File file = new File(Recorder.FILE_PATH);
        if (!file.exists()) {
            System.out.println("文件记录不存在！");
            choose = "1";
        }
        if ("1".equals(choose)) {
            // 初始化敌方坦克集合
            for (int i = 0; i < Constants.ENEMY_TANK_SIZE; i++) {
                ...
                enemyTank.setEnemyTanks(mEnemyTanks);
            }
            for (int i = 0; i < mEnemyTanks.size(); i++) {
                mEnemyTanks.get(i).setEnemyTanks(mEnemyTanks);
            }
        } else if ("2".equals(choose)) {
            Vector<Node> nodes = Recorder.getInstance().readFileToRecord();
            for (int i = 0; i < nodes.size(); i++) {
                Node node = nodes.get(i);
                EnemyTank enemyTank = new EnemyTank(Integer.parseInt(node.getX()), Integer.parseInt(node.getY()), Integer.parseInt(node.getDirection()));
                // 启动敌方坦克线程
                new Thread(enemyTank).start();
                mEnemyTanks.add(enemyTank);
                enemyTank.setEnemyTanks(mEnemyTanks);
        ...
        initBombImages();
        mRecordEnemyTank = new EnemyTank(Constants.WHOLE_GAME_WIDTH + 20, 80, Constants.TOP_DIRECTION);
        Recorder.getInstance().setEnemyTanks(mEnemyTanks);
        mAePlayWave = new AePlayWave(MUSIC_FILE_PATH);
        mAePlayWave.start();
    }
    ...
    private void drawPlayerInfo(Graphics graphics) {
        ...
        graphics.drawString("您已击毁敌方坦克数量", Constants.WHOLE_GAME_WIDTH + 20, 30);
        drawTank(graphics, mRecordEnemyTank);
        graphics.setColor(Color.BLACK);
        graphics.drawString(Recorder.getInstance().getDeadTankNum() + "", Constants.WHOLE_GAME_WIDTH + 90, 120);
    }
                ...
                if (heroShot != null && heroShot.isLive()) {
                    ...
                    for (int j = 0; j < mEnemyTanks.size(); j++) {
                        ...
                        if (tankIsShoted) {
                            ...
                            Recorder.getInstance().addDeadTankNum();
                            break;
                        }
                    }
                } else {
